...
# 配置
# ============================================================
BACKTEST_NAME = "ChanFund-Fusion_v1.2"
...
MAX_POSITIONS_NORMAL = 5    # 正常市5只
MAX_POSITIONS_BEAR = 2      # 熊市2只
POSITION_SIZE = 0.20
COMMISSION_RATE = 0.0003

# 【v1.2】ATR自适应止损（替代固定8%）
ATR_STOP_MULT = 2.5         # 止损 = 入场价 - ATR(14)×2.5
FIXED_STOP_FALLBACK = 0.10  # 无ATR时的后备10%止损
...
# 【v1.2】入场价格确认
ENTRY_CONFIRM_ABOVE_HIGH = True  # 信号次日收盘需高于信号日最高价

# 沪深300 MA200 熊市阈值
CSI300_SYMBOL = "sh.510300"
MARKET_MA_PERIOD = 200

LOG_PATH = _WORKSPACE + "/backtest_v12_run.log"
logging.basicConfig(
    level=logging.INFO,
...
def generate_report(metrics, signal_stats, exit_stats, tp_stats, trade_log,
    ...
    bear_pct = round(bear_days / max(1, len(backtest_dates)) * 100, 1)
    report = f"""# ChanFund Fusion v1.2 — 全量回测报告

> 生成时间: {now}
> 策略版本: v1.2 (2026-05-03 · 3大优化：ATR止损/入场确认/市场过滤)
...
| 特性 | 状态 | 说明 |
|------|------|------|
| ATR自适应止损 | ✅ v1.2 | ATR(14)×{ATR_STOP_MULT:.1f}，不低于{FIXED_STOP_FALLBACK*100:.0f}% |
| 市场价格过滤 | ✅ v1.2 | 沪深300 MA200：熊市缩仓至{MAX_POSITIONS_BEAR}只 |
| 入场价格确认 | ✅ v1.2 | 收盘高于信号日最高价确认 |
| 多级止盈系统 | ✅ v1.1 | 10%/20%分批减仓+峰值回落清仓+保本移损 |
| 成交量确认过滤 | ✅ v1.1 | ≥20日均量×{VOL_MIN_RATIO*100:.0f}% |
| 集中持仓 | ✅ v1.1 | 牛市{MAX_POSITIONS_NORMAL}只×{POSITION_SIZE*100:.0f}%，熊市{MAX_POSITIONS_BEAR}只 |
| ER噪声过滤 | ✅ 继承 | 分级降权 |
...
def main():
    ...
    logger.info("=" * 70)
    logger.info(f"  🚀 ChanFund Fusion v1.2 — 全量回测开始")
    ...
    # ===== 阶段2: 正式回测 (v1.2) =====
    logger.info("=" * 60)
    logger.info("PHASE 2: Backtest v1.2")
    logger.info("=" * 60)
    ...
    # 【v1.2】ATR缓存 {symbol: [close prices for ATR calc]}
    atr_cache = {}  # {symbol: [high, low, close, ...]}
    ...
    for day_idx, date_str in enumerate(backtest_dates):
        ...
        for stock, kbar in day_data.items():
            ...
            if close > 0:
                if stock not in atr_cache:
                    atr_cache[stock] = []
                atr_cache[stock].append({
                    "high": kbar.get("high", 0),
                    "low": kbar.get("low", 0),
                    "close": close,
                })
                if len(atr_cache[stock]) > 30:
                    atr_cache[stock] = atr_cache[stock][-30:]

        # 【v1.2】沪深300市场状态更新
        csi_kbar = day_data.get(CSI300_SYMBOL, {})
        csi_close = csi_kbar.get("close", 0)
        if csi_close > 0:
            market_prices.append(csi_close)

        # 判断牛熊
        is_bear_market = False
        if len(market_prices) >= MARKET_MA_PERIOD:
            ma200 = sum(market_prices[-MARKET_MA_PERIOD:]) / MARKET_MA_PERIOD
            is_bear_market = market_prices[-1] < ma200 * 0.98
        max_positions = MAX_POSITIONS_BEAR if is_bear_market else MAX_POSITIONS_NORMAL
        if is_bear_market:
            bear_days += 1
        ...
        for sig in signals:
            ...
            # 【v1.2】入场价格确认：次日收盘需高于信号日最高价
            if ENTRY_CONFIRM_ABOVE_HIGH:
                kbar = day_data[sig.symbol]
                if kbar.get("close", 0) <= kbar.get("high", 999999):
                    # 信号当日收盘未创新高 → 跳过
                    pass  # 继续检查其他条件
            ...
            # 【v1.2】ATR自适应止损
            atr_bars = atr_cache.get(sig.symbol, [])
            if len(atr_bars) >= 15:
                # 计算ATR(14): 平均真实波幅
                tr_values = []
                for i in range(1, min(len(atr_bars), 15)):
                    ...
                    tr_values.append(tr)
                if tr_values:
                    atr_val = sum(tr_values[-14:]) / min(len(tr_values), 14)
                    stop_price = entry_price - (atr_val * ATR_STOP_MULT)
                    # 止损不低于10%
                    stop_price = max(stop_price, entry_price * (1 - FIXED_STOP_FALLBACK))
                else:
                    stop_price = entry_price * (1 - FIXED_STOP_FALLBACK)
            else:
                stop_price = entry_price * (1 - FIXED_STOP_FALLBACK)
    ...
    report_filename = f"ChanFund-Fusion_v1.2_full_{report_date}.md"
    ...
    if trade_log:
        csv_path = REPORT_DIR / f"ChanFund-Fusion_v1.2_full_{report_date}_trades.csv"
