...
class TrialBuyManager:
    ...
    def update(self, symbol: str, kbar_count: int, high: float, low: float,
        ...
        # 2. 检查2R主动止盈（v1.0.1: 平一半，移损至成本）
        if high >= pos.target_price:
            pos.status = "TAKEPROFIT"
            pos.stop_price = pos.entry_price  # 剩余仓位止损移至成本
            logger.info(
                f"[{symbol}] TrialBuy 2R takeprofit: close half at {pos.target_price:.3f},"
                f" trail stop to {pos.entry_price:.3f}"
            )
            self.active[symbol] = pos  # 保留剩余半仓（止损在成本价）
            return "takeprofit_half"
