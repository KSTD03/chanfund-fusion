...
# 配置
# ============================================================
BACKTEST_NAME = "ChanFund-Fusion_v1.3"
...
# 【v1.3】市场代理：取前20大市值股票均值作为市场风向标
MARKET_PROXY_STOCKS = [
    "sh.601398", "sh.601939", "sh.601288", "sh.601988",  # 四大行
    "sh.600519", "sz.000858",                            # 茅台五粮液
    "sh.601318", "sh.601628",                            # 保险
    "sh.600036", "sh.600016",                            # 招行民生
    "sh.600900",                                         # 长江电力
    "sh.600028", "sh.600688",                            # 中石化
    "sh.600019", "sh.600010",                            # 钢铁
    "sh.600585",                                         # 海螺水泥
    "sz.000002",                                          # 万科
    "sz.000001",                                          # 平安银行
    "sh.601857",                                          # 中石油
...
MARKET_MA_MIN_STOCKS = 5  # 至少5只股票有数据才判定

LOG_PATH = _WORKSPACE + "/backtest_v13_run.log"
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger("chanfund_v13_bt")
...
def generate_report(metrics, signal_stats, exit_stats, tp_stats, trade_log,
    ...
    report = f"""# ChanFund Fusion v1.3 — 全量回测报告

> 生成时间: {now}
> 策略版本: v1.3 (2026-05-03 · 大盘20股代理+熊市缩仓)
...
| 特性 | 状态 | 说明 |
|------|------|------|
| **大盘20股代理MA200** | ✅ **v1.3** | 20只大市值股等权均值，MA200判定牛熊，熊市缩仓至{MAX_POSITIONS_BEAR}只 |
| 多级止盈系统 | ✅ v1.1 | 10%/20%分批减仓+峰值回落清仓+保本移损 |
| 成交量确认过滤 | ✅ v1.1 | ≥20日均量×{VOL_MIN_RATIO*100:.0f}% |
| 固定8%止损 | ✅ v1.1 | 统一{FIXED_STOP_LOSS*100:.0f}%硬止损+保本移损 |
| 集中持仓 | ✅ v1.1 | 牛市{MAX_POSITIONS}只×{POSITION_SIZE*100:.0f}%，熊市{MAX_POSITIONS_BEAR}只 |
| ER噪声过滤 | ✅ 继承 | 分级降权 |
...
def main():
    ...
    logger.info("=" * 70)
    logger.info(f"  🚀 ChanFund Fusion v1.3 — 全量回测开始")
    ...
    # ===== 阶段2: 正式回测 (v1.3) =====
    logger.info("=" * 60)
    logger.info("PHASE 2: Backtest v1.3")
    logger.info("=" * 60)
    ...
    # 【v1.3】大盘代理价格
    market_prices_by_stock = {s: [] for s in MARKET_PROXY_STOCKS}
    bear_days = 0
    ...
    for day_idx, date_str in enumerate(backtest_dates):
        ...
        # 【v1.3】更新大盘代理
        for proxy_stock in MARKET_PROXY_STOCKS:
            if proxy_stock in day_data:
                market_prices_by_stock[proxy_stock].append(
                    day_data[proxy_stock]["close"]
                )

        # 计算大盘MA200
        valid_closes = []
        for s in MARKET_PROXY_STOCKS:
            prices = market_prices_by_stock[s]
            if len(prices) >= MARKET_MA_PERIOD:
                valid_closes.append(prices[-1])

        is_bear_market = False
        if len(valid_closes) >= MARKET_MA_MIN_STOCKS:
            avg_close = sum(valid_closes) / len(valid_closes)
            # 计算大盘MA200
            total = 0
            count = 0
            for s in MARKET_PROXY_STOCKS:
                prices = market_prices_by_stock[s]
                if len(prices) >= MARKET_MA_PERIOD:
                    total += sum(prices[-MARKET_MA_PERIOD:]) / MARKET_MA_PERIOD
                    count += 1
            if count >= MARKET_MA_MIN_STOCKS:
                avg_ma200 = total / count
                is_bear_market = avg_close < avg_ma200 * 0.98

        max_positions = MAX_POSITIONS_BEAR if is_bear_market else MAX_POSITIONS
        if is_bear_market:
            bear_days += 1
    ...
    report_filename = f"ChanFund-Fusion_v1.3_full_{report_date}.md"
    ...
    if trade_log:
        csv_path = REPORT_DIR / f"ChanFund-Fusion_v1.3_full_{report_date}_trades.csv"
        pd.DataFrame(trade_log).to_csv(csv_path, index=False, encoding="utf-8-sig")

    logger.info(f"✅ 报告: {report_path}")
    print(f"""
{'='*55}
  🎯 ChanFund Fusion v1.3 回测完成
{'='*55}
  总收益率: {metrics.get('total_return_pct', 'N/A')}%
  夏普比率: {metrics.get('sharpe_ratio', 'N/A')}
  最大回撤: {metrics.get('max_drawdown_pct', 'N/A')}%
