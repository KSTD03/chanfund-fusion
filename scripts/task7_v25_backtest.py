...
=============================
改动：
  1. TP1_PCT = 0.08（从 0.06）
  2. tp1_triggered → tp1_done，tp2_triggered → tp2_done（允许多次触发）
  3. 尾随止损: BULL=0.88, 非BULL=0.91
  4. ENV_MULT: BULL=1.0, OSCILLATE=0.8, BEAR=0.6
  5. 股票池: 近250日均量前800只
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(str(OUTPUT_DIR / "v25_run.log"), mode="w", encoding="utf-8"),
    ],
)
logger = logging.getLogger("v25")
...
def get_fund_score(qlib_code: str, date_str: str, cache: dict) -> Optional[float]:
    """
    从 scores.parquet 缓存中获取基本面评分。
    查询 <= date_str 的最新一个 end_date 的 score。
    如果无匹配，返回 None（由调用方处理默认值 50）。
    """
    sc = cache.get(qlib_code)
    if not sc: return None
    # reversed 更高效，找到第一个 <= date_str 的就返回
...
def run_v25():
    t_start = time.time()
    logger.info(f"{'='*55}")
    logger.info(f"🔰 v2.5 全量回测")
    logger.info(f"  TP1=8% | 牛尾随0.88 | 震荡乘数0.8 | 池=800只")
    logger.info(f"{'='*55}")
    ...
    # === Phase 2: Backtest ===
    logger.info("Phase 2: Backtest v2.5...")
    ...
    pd.DataFrame(trade_log).to_csv(OUTPUT_DIR/"v25_trades.csv", index=False)
    pd.DataFrame(equity_curve).to_csv(OUTPUT_DIR/"v25_equity.csv", index=False)
    ...
    with open(OUTPUT_DIR/"v25_summary.json","w") as f:
    ...
    logger.info(f"  v2.1+MA (pre<30+共振+MA):     {metrics.get('total_return','?'):.2f}%  夏普{metrics.get('sharpe','?'):.4f}")

    # 改动说明
    logger.info(f"\n{'='*55}")
    logger.info(f"📝 v2.5 改动说明")
    logger.info(f"{'='*55}")
    logger.info(f"  参数改动:")
    logger.info(f"    TP1_PCT: 0.06 → 0.08")
    logger.info(f"    ENV_MULT[OSCILLATE]: 1.0 → 0.8")
    logger.info(f"    股票池: 全A(4441只) → 均量前800只")
    logger.info(f"  逻辑改动:")
    logger.info(f"    tp1_triggered/tp2_triggered → tp1_done/tp2_done（允许多次触发）")
    logger.info(f"    尾随止损: BULL=0.88(回落12%), 非BULL=0.91(回落9%)")
    logger.info(f"  不改的:")
    logger.info(f"    FUND_SCORE_MIN=30, POSITION_SIZE=0.20, FIXED_STOP_LOSS=0.08")
    logger.info(f"    TP2_PCT=0.13, TP_REDUCE不变, 保本锁=5%, 冷却期不变")
    logger.info(f"    环境分类器: MA20/60不变, 波动率保护不变")
