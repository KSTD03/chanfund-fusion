#!/usr/bin/env python3
"""
v6-C — 方向B: 环境+板块共向性 (趋势对齐)
========================================
基于 v4.0（含滑点/涨跌停/成交量限制）

核心思想：买入信号→检查个股趋势是否与宏观环境对齐
...
OUTPUT_DIR = Path("/home/quant/.openclaw/workspace/output/v6c_ns")
...
logger = logging.getLogger("v6c_ns")
...
def run_v6c():
    t0=time.time()
    logger.info(f"{'='*55}")
    logger.info(f"🔰 v6-C-NS — 环境+趋势对齐 (方向B, 无滑点)")
    ...
    logger.info(f"\n{'='*55}")
    logger.info(f"📊 v6-C 结果 — 环境+趋势对齐 (方向B)")
    logger.info(f"{'='*55}")
    ...
    pd.DataFrame(trade_log).to_csv(OUTPUT_DIR/"v6c_trades.csv",index=False)
    pd.DataFrame(equity_curve).to_csv(OUTPUT_DIR/"v6c_equity.csv",index=False)
    ...
    with open(OUTPUT_DIR/"v6c_summary.json","w") as f:json.dump(ms,f,indent=2,ensure_ascii=False)
