#!/usr/bin/env python3
"""
v6.5a-S_czsc-F_thsMmt-C_real-R_r4-riskparity
控制实验: riskparity + minhold3 + hdOnly + BULL
"""
...
OUT_DIR=Path(WORKSPACE)/"output/v6.5a-S_czsc-F_thsMmt-C_real-R_r4-riskparity"
...
analysis={"total_return_pct":round(tot,2),"ann_return_pct":round(ann_ret,2),"sharpe":round(sharpe,4),"max_drawdown_pct":round(md,2),"win_rate_pct":round(wr,1),"n_buys":len(trades_df[trades_df["action"]=="BUY"]),"n_sells":len(sells),"profit_loss_ratio":round(wlr,2),"avg_win_pct":round(aw,2),"avg_loss_pct":round(al,2),"momentum_passed":mpa,"momentum_rejected":mre,"r4_filtered":bep,"yearly_returns":yearly,"exit_reasons":exit_analysis,"signal_types":dict(sig_stats),"backtest_start":bt_start,"backtest_end":end_d,"final_nav":round(eq_df.iloc[-1]["nav"],2),"elapsed_seconds":round(time.time()-t0,1),}
