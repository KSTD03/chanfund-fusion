#!/usr/bin/env python3
"""
v6.5b-S_czsc-F_thsMmt_C_mom70-C_real-R_full-bullOnly
控制实验: 动量>70% + BULL环境
"""
...
OUT_DIR=Path(WORKSPACE)/"output/v6.5b-S_czsc-F_thsMmt_C_mom70-C_real-R_full-bullOnly"
...
analysis={"total_return_pct":round(tot,2),"ann_return_pct":round(ann_ret,2),"sharpe":round(sharpe,4),"max_drawdown_pct":round(md,2),"win_rate_pct":round(wr,1),"n_buys":len(trades_df[trades_df["action"]=="BUY"]),"n_sells":len(sells),"profit_loss_ratio":round(wlr,2),"avg_win_pct":round(aw,2),"avg_loss_pct":round(al,2),"momentum_passed":mpa,"momentum_rejected":mre,"mom70_filtered":bep,"yearly_returns":yearly,"exit_reasons":exit_analysis,"signal_types":dict(sig_stats),"backtest_start":bt_start,"backtest_end":end_d,"final_nav":round(eq_df.iloc[-1]["nav"],2),"elapsed_seconds":round(time.time()-t0,1),}
...
print(f"  动量通过: {mpa} 拒绝: {mre} | mom70过滤: {bep}")
