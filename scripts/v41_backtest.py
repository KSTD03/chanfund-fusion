...
OUTPUT_DIR = Path("/home/quant/.openclaw/workspace/output/v41")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

import logging
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    datefmt="%H:%M:%S", handlers=[logging.StreamHandler(),
        logging.FileHandler(str(OUTPUT_DIR / "v41_run.log"), mode="w", encoding="utf-8")])
...
FIXED_STOP_LOSS, FUND_SCORE_MIN = 0.08, 60.0  # 仅改了这个：30→60
...
def run_v41():
    t0=time.time()
    logger.info(f"{'='*55}")
    logger.info(f"🔰 v4.1 — fund_score >= 60 过滤")
    logger.info(f"  滑点买×{SLIPPAGE_BUY}卖×{SLIPPAGE_SELL} | 涨跌停 | 成交量5% | scores≥60")
    logger.info(f"{'='*55}")
    ...
    for di,ds in enumerate(bt_dates):
        ...
        for sig in signals:
            ...
            if pc>0:
                if entry_price>=round(pc*1.10,2) or entry_price<=round(pc*0.90,2): continue
            ...
            if fund_score<FUND_SCORE_MIN: score_stats["rejected"]+=1; continue  # scores>=60
    ...
    pd.DataFrame(trade_log).to_csv(OUTPUT_DIR/"v41_trades.csv",index=False)
    pd.DataFrame(equity_curve).to_csv(OUTPUT_DIR/"v41_equity.csv",index=False)
    ms={k:v for k,v in metrics.items() if isinstance(v,(int,float,str))}
    ms.update({"win_rate":round(wr,1),"profit_factor":round(pf,2)})
    with open(OUTPUT_DIR/"v41_summary.json","w") as f: json.dump(ms,f,indent=2,ensure_ascii=False)
