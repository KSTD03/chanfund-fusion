#!/usr/bin/env python3
"""
v6.5a-S_czsc-F_thsMmt-C_real-R_r5-optfilt
控制实验: optfilt + riskparity + minhold3 + hdOnly + BULL
"""
...
OUT_DIR=Path(WORKSPACE)/"output/v6.5a-S_czsc-F_thsMmt-C_real-R_r5-optfilt"
...
SLIPPAGE_BUY,SLIPPAGE_SELL=1.001,0.999; VOL_CAP_RATIO,FIXED_STOP_LOSS=0.05,0.08; FUND_SCORE_MIN=38.0
...
for di,ds in enumerate(bd):
    ...
    for sig in signals:
        ...
        # 移除: 动量过滤（与hard_divergence反转信号原理矛盾）
        mpa+=1
        vols=vc.get(sig.symbol,[])
        if len(vols)>=20:
            av2=sum(vols[-20:])/20
            if av2>0 and dd[sig.symbol].get("volume",0)<av2*VOL_MIN_RATIO: continue
        # Risk parity: risk_amount / risk_per_share
        atr_price=calc_atr(sh[sig.symbol])*ep_d # ATR换算为元
        risk_per_share=max(atr_price,ep_d*0.02) # 最低2%价格波动
        shares_raw=(cash*0.04)/max(risk_per_share,0.01)
        max_by_cap=cash*0.40/(ep_d*SLIPPAGE_BUY)
        dv=data.get(nd,{}).get(sig.symbol,{}).get("volume",0)
        max_by_vol=int(dv*VOL_CAP_RATIO/100)*100 if dv>0 else 9999999
        sh2=int(min(shares_raw,max_by_cap,max_by_vol)/100)*100
        if sh2<=0: continue
...
analysis={"total_return_pct":round(tot,2),"ann_return_pct":round(ann_ret,2),"sharpe":round(sharpe,4),"max_drawdown_pct":round(md,2),"win_rate_pct":round(wr,1),"n_buys":len(trades_df[trades_df["action"]=="BUY"]),"n_sells":len(sells),"profit_loss_ratio":round(wlr,2),"avg_win_pct":round(aw,2),"avg_loss_pct":round(al,2),"momentum_passed":mpa,"momentum_rejected":mre,"r5_filtered":bep,"yearly_returns":yearly,"exit_reasons":exit_analysis,"signal_types":dict(sig_stats),"backtest_start":bt_start,"backtest_end":end_d,"final_nav":round(eq_df.iloc[-1]["nav"],2),"elapsed_seconds":round(time.time()-t0,1),}
