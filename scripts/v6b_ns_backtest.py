#!/usr/bin/env python3
"""
v6-B — 小改动: czsc min_bi_len=6 + 顶底无包含校验
=================================================
基于 v4.0（含滑点/涨跌停/成交量限制）

改动：
  1. bi_min_kbar=6 (vs 5) — 更严格的成笔条件
  2. 顶底无包含校验 — BI的起始分型和结束分型不能有价格区间包含关系
  3. 其余完全同 v4.0
"""
...
OUTPUT_DIR = Path("/home/quant/.openclaw/workspace/output/v6b_ns")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

import logging
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    datefmt="%H:%M:%S", handlers=[logging.StreamHandler(),
        logging.FileHandler(str(OUTPUT_DIR / "v6b_run.log"), mode="w", encoding="utf-8")])
logger = logging.getLogger("v6b_ns")
...
# ===== v6-B: czsc 移植 =====
BI_MIN_KBAR = 6  # czsc 默认值

def has_fx_inclusion(start_fx, end_fx):
    """检查起始分型和结束分型是否有价格区间包含关系
    
    czsc 的 check_bi() 逻辑:
    起始bar和结束bar不能有包含关系:
    - start.high <= end.high AND start.low >= end.low → start被end包含
    - end.high <= start.high AND end.low >= start.low → end被start包含
    
    在我们的引擎中，FX有high/low属性（分型所在K线的OHLC）
    """
    if start_fx is None or end_fx is None:
        return True  # 数据不全视为包含（拒绝）
    
    # start 包含 end
    if start_fx.high >= end_fx.high and start_fx.low <= end_fx.low:
        return True
    # end 包含 start
    if end_fx.high >= start_fx.high and end_fx.low <= start_fx.low:
        return True
...
def run_v6b():
    t0=time.time()
    logger.info(f"{'='*55}")
    logger.info(f"🔰 v6-B-NS — czsc移植: min_bi_len=6 + 顶底无包含 (无滑点)")
    logger.info(f"  bi_min_kbar={BI_MIN_KBAR}")
    logger.info(f"{'='*55}")
    ...
    from chanfund_fusion.tech.chan_structure import ChanStructureState, BiConfirmedEvent
    from chanfund_fusion.tech.chan_objects import FxType
    
    import yaml
    with open(str(Path("/home/quant/.openclaw/workspace/chanfund_fusion/config.yaml")), 'r') as f:
        cfg = yaml.safe_load(f)
    
    # === 改动1: bi_min_kbar=6 ===
    cfg.setdefault("tech", {})["bi_min_kbar"] = BI_MIN_KBAR
    tech_engine = TechSignalEngine(cfg.get("tech", {}))
    
    # === 改动2: 顶底无包含校验 — monkey patch ===
    original_confirm_bi = ChanStructureState._confirm_bi
    inclusion_rejected_bi = 0  # 统计
    
    def patched_confirm_bi(self, new_fx, kbar):
        nonlocal inclusion_rejected_bi
        bi = original_confirm_bi(self, new_fx, kbar)
        # bi是被确认完成的笔
        if bi is not None:
            # 检查顶底无包含
            if has_fx_inclusion(bi.start_fx, bi.end_fx):
                # 有包含关系 → 从confirmed_bis中移除这笔
                inclusion_rejected_bi += 1
                if bi in self.confirmed_bis:
                    self.confirmed_bis.remove(bi)
                # 注意：current_bi 已经创建了新笔，不影响
                return None  # 返回None表示这笔被拒绝
        return bi

    ChanStructureState._confirm_bi = patched_confirm_bi
    ...
    logger.info(f"✅ 预热完成 (包含拒绝={inclusion_rejected_bi})")
    ...
    logger.info(f"\n{'='*55}")
    logger.info(f"📊 v6-B 结果 — czsc移植 (bi_min_kbar=6, 顶底无包含)")
    logger.info(f"{'='*55}")
    ...
    logger.info(f"  包含拒绝: {inclusion_rejected_bi}")
    logger.info(f"  环境: {dict(sorted(env_counts.items()))}   scores: {score_stats}")
    pd.DataFrame(trade_log).to_csv(OUTPUT_DIR/"v6b_trades.csv",index=False)
    pd.DataFrame(equity_curve).to_csv(OUTPUT_DIR/"v6b_equity.csv",index=False)
    ms={k:v for k,v in metrics.items() if isinstance(v,(int,float,str))}
    ms.update({"win_rate":round(wr,1),"profit_factor":round(pf,2),
               "inclusion_rejected":inclusion_rejected_bi,"bi_min_kbar":BI_MIN_KBAR})
    with open(OUTPUT_DIR/"v6b_summary.json","w") as f:json.dump(ms,f,indent=2,ensure_ascii=False)
