...
OUTPUT_DIR = Path("/home/quant/.openclaw/workspace/output/v5c")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

import logging
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    datefmt="%H:%M:%S", handlers=[logging.StreamHandler(),
        logging.FileHandler(str(OUTPUT_DIR / "v5c_run.log"), mode="w", encoding="utf-8")])
logger = logging.getLogger("v5c")
...
def run_v5c():
    t0=time.time()
    logger.info(f"{'='*55}")
    logger.info(f"🔰 v5-C — BEAR空仓")
    logger.info(f"  BEAR→不开新仓 | 仅BULL+OSCILLATE交易")
    logger.info(f"{'='*55}")
    ...
    bear_no_trade_count=0

    logger.info("Backtest v5-C...")
    ...
    logger.info(f"  BEAR空仓拒绝: {bear_no_trade_count}笔")
    logger.info(f"  scores: {score_stats}  环境: {dict(sorted(env_counts.items()))}")
    pd.DataFrame(trade_log).to_csv(OUTPUT_DIR/"v5c_trades.csv",index=False)
    pd.DataFrame(equity_curve).to_csv(OUTPUT_DIR/"v5c_equity.csv",index=False)
    ms={k:v for k,v in metrics.items() if isinstance(v,(int,float,str))}
    ms.update({"win_rate":round(wr,1),"profit_factor":round(pf,2)})
    with open(OUTPUT_DIR/"v5c_summary.json","w") as f: json.dump(ms,f,indent=2,ensure_ascii=False)
