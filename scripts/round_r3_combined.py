...
===========================
设计 (2026-05-11 22:22):
  - R1: 环境依赖 TP/SL（BULL/OSCILLATE/BEAR 不同止盈止损）
...
ROUND = "R3"
OUTPUT_DIR = Path(f"/home/quant/.openclaw/workspace/output/r3")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

VOL_FILTER_RATIO = 0.80
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(str(OUTPUT_DIR / "r3_run.log"), mode="w", encoding="utf-8"),
    ],
)
...
class VolumeFilter:
    ...
    def check(self, stock: str, ds: str, day_data: dict) -> bool:
        ...
        if len(vols) < 20: return True
        ma20 = sum(vols[-20:]) / 20
        today_vol = day_data.get("volume", 0)
        if ma20 <= 0 or today_vol <= 0: return True
        if today_vol / ma20 < self.ratio:
...
def run_r3():
    t_start = time.time()
    logger.info(f"{'='*55}")
    logger.info(f"🔰 R3 — R1(环境TP/SL) + R2(成交量过滤) 组合")
    logger.info(f"  BULL: TP1=12%, trail=0.88 | OSC: TP1=6%, trail=0.94 | BEAR: TP1=3%, stop=3%")
    logger.info(f"  成交量过滤: volume > {VOL_FILTER_RATIO*100:.0f}% MA20")
    logger.info(f"{'='*55}")
    ...
    logger.info("Phase 2: Backtest R3...")
    ...
    logger.info(f"  R3: R1(环境TP/SL) + R2(成交量过滤{VOL_FILTER_RATIO}), 成交量被拒={vol_filter_obj.rejected_count}")
    logger.info(f"  R1参数: BULL(TP1=12%,TP2=18%,trail=0.88) OSC(TP1=6%,TP2=10%,trail=0.94) BEAR(TP1=3%,TP2=5%,stop=3%,no_trail)")

    pd = __import__('pandas')
    pd.DataFrame(trade_log).to_csv(OUTPUT_DIR/"r3_trades.csv", index=False)
    pd.DataFrame(equity_curve).to_csv(OUTPUT_DIR/"r3_equity.csv", index=False)
    ...
    with open(OUTPUT_DIR/"r3_summary.json","w") as f:
