#!/usr/bin/env python3
"""
v6.5a-S_czsc-F_thsMmt-C_real-R_sz5
控制实验: 仓位5% + hdOnly + BULL环境
"""
...
OUT_DIR=Path(WORKSPACE)/"output/v6.5a-S_czsc-F_thsMmt-C_real-R_sz5"
...
log=logging.getLogger("v65a_sz5")
...
for di,ds in enumerate(bd):
    ...
    for sig in signals:
        ...
        ra=(cash*0.05*vf*fm)/max(sa,0.01);ra=min(ra,cash*0.40)
...
analysis={"total_return_pct":round(tot,2),"ann_return_pct":round(ann_ret,2),"sharpe":round(sharpe,4),"max_drawdown_pct":round(md,2),"win_rate_pct":round(wr,1),"n_buys":len(trades_df[trades_df["action"]=="BUY"]),"n_sells":len(sells),"profit_loss_ratio":round(wlr,2),"avg_win_pct":round(aw,2),"avg_loss_pct":round(al,2),"momentum_passed":mpa,"momentum_rejected":mre,"sz5_filtered":bep,"yearly_returns":yearly,"exit_reasons":exit_analysis,"signal_types":dict(sig_stats),"backtest_start":bt_start,"backtest_end":end_d,"final_nav":round(eq_df.iloc[-1]["nav"],2),"elapsed_seconds":round(time.time()-t0,1),}
...
print(f"  动量通过: {mpa} 拒绝: {mre} | sz5过滤: {bep}")
