...
"""
v2.6 — 基于 R1 版，仅改两个参数：
  1. ENV_MULT: BULL=1.0, OSCILLATE=0.8, BEAR=0.6 (同R1，确认)
  2. BULL: TP1=10%, TP2=15% (R1: TP1=12%, TP2=18%)
"""
...
OUTPUT_DIR = Path("/home/quant/.openclaw/workspace/output/v26")
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(str(OUTPUT_DIR / "v26_run.log"), mode="w", encoding="utf-8"),
    ],
)
logger = logging.getLogger("v26")
...
def get_env_params(env: str):
    if env == "BULL":
        return {"tp1": 0.10, "tp2": 0.15, "trail": 0.88, "env_stop": None}
    elif env == "OSCILLATE":
...
def run_v26():
    t_start = time.time()
    logger.info(f"{'='*55}")
    logger.info(f"🔰 v2.6 — BULL TP1=10%, TP2=15%")
    logger.info(f"  BULL: TP1=10%, TP2=15%, trail=0.88")
    logger.info(f"  OSCILLATE: TP1=6%, TP2=10%, trail=0.94 (同R1)")
    logger.info(f"  BEAR: TP1=3%, TP2=5%, 硬止损3%, 无尾随 (同R1)")
    logger.info(f"  仓位乘数: BULL=1.0, OSC=0.8, BEAR=0.6 (同R1)")
    logger.info(f"{'='*55}")
    ...
    # Phase 2: Backtest
    logger.info("Phase 2: Backtest v2.6...")
    ...
    logger.info(f"  v2.6参数: BULL(TP1=10%,TP2=15%,trail=0.88) OSC(TP1=6%,TP2=10%,trail=0.94) BEAR(TP1=3%,TP2=5%,stop=3%,no_trail)")
    logger.info(f"  ENV_MULT: BULL=1.0, OSC=0.8, BEAR=0.6")

    # 产出
    pd = __import__('pandas')
    pd.DataFrame(trade_log).to_csv(OUTPUT_DIR/"v26_trades.csv", index=False)
    pd.DataFrame(equity_curve).to_csv(OUTPUT_DIR/"v26_equity.csv", index=False)
    ...
    with open(OUTPUT_DIR/"v26_summary.json","w") as f:
