...
===========================
设计 (2026-05-11 22:22):
  - BULL:   TP=12%, 尾随0.88 (同v2.5)
  - OSCILLATE: TP=6%, 尾随0.94 (收紧)
  - BEAR:   TP=3% (收紧), 止损=3% (硬止损), 无尾随
  - 其余逻辑完全同 v2.5（scores pre<30过滤、共振、池=800只）
"""
...
ROUND = "R1"
OUTPUT_DIR = Path(f"/home/quant/.openclaw/workspace/output/r1")
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(str(OUTPUT_DIR / "r1_run.log"), mode="w", encoding="utf-8"),
    ],
)
...
def get_env_params(env: str):
    """
    R1 环境依赖参数：
    BULL:    TP1=12%, TP2=18%, 尾随0.88
    OSCILLATE: TP1=6%, TP2=10%, 尾随0.94
    BEAR:    TP1=3%, TP2=5%, 止损3%（无尾随）
    """
    if env == "BULL":
...
def run_r1():
    t_start = time.time()
    logger.info(f"{'='*55}")
    logger.info(f"🔰 R1 — 环境依赖TP/SL")
    logger.info(f"  BULL: TP1=12%, 尾随0.88")
    logger.info(f"  OSCILLATE: TP1=6%, 尾随0.94")
    logger.info(f"  BEAR: TP1=3%, 硬止损3%, 无尾随")
    logger.info(f"{'='*55}")
    ...
    # Phase 2: Backtest
    logger.info("Phase 2: Backtest R1...")
    for di, ds in enumerate(bt_dates):
        ...
        ep = get_env_params(env)  # R1 环境依赖参数
        ...
        for stock, pos in list(positions.items()):
            ...
            # === R1: 环境依赖止损 ===
            if ep["env_stop"] is not None:  # BEAR: 硬止损3%
    ...
    pd.DataFrame(trade_log).to_csv(OUTPUT_DIR/"r1_trades.csv", index=False)
    pd.DataFrame(equity_curve).to_csv(OUTPUT_DIR/"r1_equity.csv", index=False)
    ...
    with open(OUTPUT_DIR/"r1_summary.json","w") as f:
