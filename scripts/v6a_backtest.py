#!/usr/bin/env python3
"""
v6-A — 方向A: 信号质量提纯 (BI力度过滤)
========================================
基于 v4.0（含滑点/涨跌停/成交量限制）

改动：
  1. 提取缠论引擎的 BI 力度（笔长度、振幅）
  2. 滚动窗口底部 30% 的弱笔信号拒绝
  3. 其余完全同 v4.0
"""
...
OUTPUT_DIR = Path("/home/quant/.openclaw/workspace/output/v6a")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

import logging
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    datefmt="%H:%M:%S", handlers=[logging.StreamHandler(),
        logging.FileHandler(str(OUTPUT_DIR / "v6a_run.log"), mode="w", encoding="utf-8")])
logger = logging.getLogger("v6a")
...
# ===== 方向A 参数 =====
BI_POWER_WINDOW = 60        # 滚动窗口: 记录最近60个信号的BI力度
BI_POWER_PERCENTILE = 30    # 拒绝底部30%信号
...
# ===== 方向A: BI力度提取 =====
def get_bi_power(tech_engine, symbol):
    """提取当前最近一笔的力度
    
    Returns:
        (bi_length, bi_amplitude, bi_score): 笔长度(K线数), 振幅, 综合评分
        如果无笔数据返回 (0, 0, 0)
    """
    state = tech_engine.get_structure_state(symbol)
    if not state: return (0, 0, 0)
    bis = state.confirmed_bis
    if not bis: return (0, 0, 0)
    
    # 取当前最新完成的笔
    last_bi = bis[-1]
    bi_length = last_bi.end_index - last_bi.start_index
    bi_amplitude = last_bi.amplitude if hasattr(last_bi, 'amplitude') else (last_bi.high - last_bi.low)
    
    # 综合评分: 长度 × 振幅
    # 长度越大说明趋势越稳, 振幅越大说明力度越强
    bi_score = max(1, bi_length) * bi_amplitude
    return (bi_length, bi_amplitude, bi_score)

def run_v6a():
    t0=time.time()
    logger.info(f"{'='*55}")
    logger.info(f"🔰 v6-A — BI力度过滤 (方向A)")
    logger.info(f"  窗口={BI_POWER_WINDOW}笔, 拒绝底部{BI_POWER_PERCENTILE}%")
    logger.info(f"{'='*55}")
    ...
    # ===== 方向A: 滚动BI力度窗口 =====
    bi_power_history = []  # 存储最近 N 个信号的 BI 评分
    bi_power_rejected = 0
    bi_power_passed = 0

    logger.info("Backtest v6-A...")
    for di,ds in enumerate(bt_dates):
        ...
        for sig in signals:
            ...
            # ===== 方向A: BI力度过滤 =====
            bi_len, bi_amp, bi_score = get_bi_power(tech_engine, sig.symbol)
            if bi_score > 0 and len(bi_power_history) >= BI_POWER_WINDOW:
                # 计算滚动阈值（底部30%分位）
                threshold = np.percentile(bi_power_history[-BI_POWER_WINDOW:], BI_POWER_PERCENTILE)
                if bi_score < threshold:
                    bi_power_rejected += 1
                    continue  # 拒绝
            bi_power_passed += 1
            bi_power_history.append(bi_score)
            # 保持窗口大小
            if len(bi_power_history) > BI_POWER_WINDOW * 2:
                bi_power_history = bi_power_history[-BI_POWER_WINDOW:]
            ...
            trade_log.append({"date":ds,"symbol":sig.symbol,"action":"BUY",
                ...
                "fund_score":fund_score,"env":env,"bi_score":round(bi_score,2)})
    ...
    logger.info(f"  BI力度: 通过={bi_power_passed} 拒绝={bi_power_rejected} 通过率={bi_power_passed/max(bi_power_passed+bi_power_rejected,1)*100:.0f}%")
    logger.info(f"  环境: {dict(sorted(env_counts.items()))}   scores: {score_stats}")
    pd.DataFrame(trade_log).to_csv(OUTPUT_DIR/"v6a_trades.csv",index=False)
    pd.DataFrame(equity_curve).to_csv(OUTPUT_DIR/"v6a_equity.csv",index=False)
    ms={k:v for k,v in metrics.items() if isinstance(v,(int,float,str))}
    ms.update({"win_rate":round(wr,1),"profit_factor":round(pf,2),"bi_power_passed":bi_power_passed,"bi_power_rejected":bi_power_rejected})
    with open(OUTPUT_DIR/"v6a_summary.json","w") as f:json.dump(ms,f,indent=2,ensure_ascii=False)
