#!/usr/bin/env python3
"""
v6.5-S_czsc-F_thsMmt-C_real-R_full-bullOnly
控制实验: 仅 THS 行业环境为 BULL 时才开仓
"""
...
OUT_DIR=Path(WORKSPACE)/"output/v6.5-S_czsc-F_thsMmt-C_real-R_full-bullOnly"
...
def load_ths():
    ...
    em={}; v=0
    for c in ic:
        sub=ths_daily[ths_daily['ts_code']==c].sort_values('trade_date')
        if len(sub)<200: continue
        cl=[]; es={}
        for _,r in sub.iterrows():
            dt=r['trade_date'];ds=f"{dt[:4]}-{dt[4:6]}-{dt[6:]}"
            cl.append(r['close'])
            if len(cl)<120: es[ds]="OSCILLATE";continue
            m20=np.mean(cl[-20:]);m60=np.mean(cl[-60:]);m120=np.mean(cl[-120:])
            es[ds]="BULL" if m20>m60>m120 else ("BEAR" if m20<m60<m120 else "OSCILLATE")
        em[c]=es;v+=1
    log.info(f"  THS: {v}个行业有效"); return em,s2t
...
analysis={"total_return_pct":round(tot,2),"ann_return_pct":round(ann_ret,2),"sharpe":round(sharpe,4),"max_drawdown_pct":round(md,2),"win_rate_pct":round(wr,1),"n_buys":len(trades_df[trades_df["action"]=="BUY"]),"n_sells":len(sells),"profit_loss_ratio":round(wlr,2),"avg_win_pct":round(aw,2),"avg_loss_pct":round(al,2),"momentum_passed":mpa,"momentum_rejected":mre,"bull_only_rejected":bep,"yearly_returns":yearly,"exit_reasons":exit_analysis,"signal_types":dict(sig_stats),"backtest_start":bt_start,"backtest_end":end_d,"final_nav":round(eq_df.iloc[-1]["nav"],2),"elapsed_seconds":round(time.time()-t0,1),}
